...
2. **Unwrap defensively.** Offload-to-file is the **normal** path here — anything past
   ...
   a path almost always. *(Persist-to-file is **client-specific**: Claude Code saves it + shows
   a preview; **CoWork** saves it to a host path you must translate to the VM mount — see
   **Running in CoWork** below; some MCP clients lose oversized responses entirely. Always read
   it with code, never inline.)* The
...
## Running in CoWork (host specifics)

CoWork runs this in a local VM, which changes three mechanics — handle them up front to avoid
wasted steps (each was a real time-sink in testing):

1. **Install the analysis libs first** — DuckDB/pyarrow are *not* preinstalled:
   ```bash
   pip install duckdb pyarrow --break-system-packages -q
   ```
2. **Locating a spilled (offloaded) MCP result — the #1 time-sink.** When
   `execute_graphql_query` overflows it returns:
   `Error: result (N characters) exceeds maximum allowed tokens. Output has been saved to <PATH>`.
   That `<PATH>` is a **host** path (`/var/folders/.../claude-hostloop-plugins/.../tool-results/…`)
   and is **NOT readable inside the VM** — don't `ls`/`cat` it. The same file is mounted under
   your working folder; grab the newest directly:
   ```bash
   ls -t /sessions/*/mnt/.claude/projects/*/*/tool-results/* 2>/dev/null | head -3
   ```
   (Layout: `…/mnt/.claude/projects/<encoded-cwd>/<conversation-id>/tool-results/mcp-…`.) Then
   parse that file with Step 3's defensive unwrap.
3. **Sub-agents work in CoWork** — delegate the bulk fetch+grid (Step 3) as written so the
   orchestrator stays clean. Write any output files to the working-folder **root** (CoWork's
   file UI only lists root-level files).

> Aggregate in **DuckDB SQL over the Parquet**, not hand-written Python loops — fewer steps and
> it avoids the fiddly f-string/`sed` edits that cost retries in testing.
